...
CodeWise is a secure, AI-powered code review assistant designed to act as a senior software engineer on demand. Built with React, Vite, and Vercel Serverless Functions, this tool bridges the gap between raw code and professional feedback. It provides instant, structured analysis focusing on readability, performance, security, and best practices.
...
## Features
  ...
  * **Serverless Security:** Utilizes a Vercel Functions backend to proxy requests to OpenAI, ensuring your API keys are never exposed to the client-side browser.
  ...
  * **Binary Protection:** Implements "Magic Byte" detection on file uploads to ensure only legitimate text files are processed, blocking malicious executables disguised as text files.

\![Screenshot: Analysis Result Dashboard]
...
**1. Input Sanitization (The Firewall)**
...
1.  **The Gatekeeper:** The frontend validates file extensions and checks magic bytes.
2.  **The Relay:** The Vercel backend receives the request, sanitizes the input strings, and constructs the payload.
3.  **The System Persona:** We inject a specialized System Prompt that instructs the AI to adopt the persona of a critical Senior Software Engineer.
4.  **Schema Enforcement:** The model is constrained to output **only** valid JSON. This ensures the frontend can reliably parse the data into the UI components (Summary, Lists, and Code Blocks) without hallucinated formatting.

\![Screenshot: Refactored Code View]
